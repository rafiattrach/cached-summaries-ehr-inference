        ...
        for k in batch[0].keys():
            if k not in ("dynamic", "static_values", "static_indices", "static_mask"):
                if isinstance(batch[0][k], datetime):
                    ...
                    # Preserve integer precision for identifier/index fields
                    if k in ("subject_id", "start_idx", "end_idx"):
                        tensorized[k] = torch.tensor([int(item[k]) for item in batch], dtype=torch.long)
                    else:
